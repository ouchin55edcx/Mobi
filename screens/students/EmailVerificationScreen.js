...
const EmailVerificationScreen = ({
  ...
  const initializeVerification = async () => {
    try {
      setLoading(true);
      const result = await createVerificationCode(studentId, email);
      if (result.error) {
        Alert.alert(
  ...
  const handleVerify = async (verificationCode = null) => {
    ...
    try {
      const result = await verifyCode(studentId, codeToVerify);
  ...
  const handleResend = async () => {
    ...
    try {
      const result = await resendVerificationCode(studentId, email);
