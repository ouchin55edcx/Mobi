...
import { getDriverTripsWithStudents } from "../../shared/services/tripTrackingService";
  ...
  const loadData = async () => {
    try {
      ...
      if (isDemo) {
        ...
        // Load from API
        const { data, error } = await getDriverTripsWithStudents(driverId, {
        ...
        if (error) {
          ...
          const transformedTrips = Object.values(data).map((trip) => ({
            id: trip.id,
            date:
              trip.reach_pickup_time ||
              trip.created_at ||
              new Date().toISOString(),
            status: trip.status?.toLowerCase() || "completed",
            school: trip.destination || "Unknown School",
            students: trip.studentCount || trip.students?.length || 0,
            duration: trip.totalDurationMin || 30,
            distance: trip.totalDistanceKm || 5.0,
          }));

          // Calculate statistics from trips
          const totalTrips = transformedTrips.length;
          const totalDistance = transformedTrips.reduce(
            (sum, t) => sum + (t.distance || 0),
            0,
          );
          const totalTime = transformedTrips.reduce(
